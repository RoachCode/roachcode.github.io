// index.js
import { mountResponsiveBackground } from './celticBox.js';
import { celticBackgroundData } from './config.js';
import { renderCharacterPage, renderMapPage, renderBattlePage } from './pages.js';
import { renderGlossaryPage } from './glossary.js';

// 1. Grab DOM elements
const bgLayer = document.getElementById('background-layer');
const pageContent = document.getElementById('page-content');
const navButtons = document.querySelectorAll('#sheet-nav-right button');

// --- NEW: Global State for Fetched Data ---
let cachedGlossaryData = null;

async function fetchGlossaryData() {
    try {
        const response = await fetch('./glossary.json');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        cachedGlossaryData = await response.json();
    } catch (error) {
        console.error("Failed to load glossary data:", error);
    }
}

// 2. Draw the static background ONCE
mountResponsiveBackground(celticBackgroundData, bgLayer);

// 3. The Tab Switching Logic
function loadTab(tabName) {
    // Wipe the container's contents safely
    pageContent.replaceChildren();

    // Update the visual "active" state on the tabs
    navButtons.forEach(btn => {
        if (btn.getAttribute('data-target') === tabName) {
            btn.classList.add('active');
        } else {
            btn.classList.remove('active');
        }
    });

    // Route to the correct page builder
    switch (tabName) {
        case 'character':
            renderCharacterPage(pageContent);
            break;
        case 'map':
            renderMapPage(pageContent);
            break;
        case 'battle':
            renderBattlePage(pageContent);
            break;
        case 'glossary':
            // Check if the background fetch has finished
            if (cachedGlossaryData) {
                renderGlossaryPage(pageContent, cachedGlossaryData); 
            } else {
                // Fallback in case they click it before the JSON request completes
                pageContent.innerHTML = `<div style="color: white; padding: 20px;">Loading Archive Data...</div>`;
            }
            break;
        default:
            break;
    }
}

// 4. Attach Event Listeners to Buttons
navButtons.forEach(button => {
    button.addEventListener('click', (e) => {
        const targetTab = e.currentTarget.getAttribute('data-target');
        loadTab(targetTab);
    });
});

// 5. Initialize the default view and fetch data on load
document.addEventListener('DOMContentLoaded', () => {
    fetchGlossaryData(); // Start grabbing the JSON in the background
    loadTab('character'); // Boot directly into the character page
});

if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
        navigator.serviceWorker.register('./sw.js')
        .then(registration => console.log('Service Worker registered'))
        .catch(error => console.error('Service Worker failed', error));
    });
}